...
impl ApplySystemConfig {
    ...
    /// Get the default config file path for the app
    pub fn default_path() -> PathBuf {
        // This will be in app data directory
        // e.g., ~/.config/gluon-desktop/apply_system_config.json
        let mut path = dirs::config_dir().unwrap_or_else(|| PathBuf::from("."));
        path.push("gluon-desktop");
        path.push("apply_system_config.json");
        path
    }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.performance.min_confidence_threshold < 0.0
            || self.performance.min_confidence_threshold > 1.0
        {
            return Err("min_confidence_threshold must be between 0.0 and 1.0".to_string());
        }

        // Check search range is reasonable
        if self.performance.fuzzy_search_range > 1000 {
            return Err("fuzzy_search_range too large (max 1000)".to_string());
        }

        // Check concurrent applies is reasonable
        if self.performance.max_concurrent_applies == 0 {
            return Err("max_concurrent_applies must be at least 1".to_string());
        }

        if self.performance.max_concurrent_applies > 16 {
            return Err("max_concurrent_applies too large (max 16)".to_string());
        }
