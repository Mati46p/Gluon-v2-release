...
//! Now supports per-change snapshots for selective undo.

use crate::apply_system::shared::types::SnapshotData;
use std::collections::HashMap;
use std::sync::{Arc, Mutex};

/// Global snapshot manager
///
/// Stores snapshots both per-file (legacy) and per-change (for selective undo)
/// Cleared when app closes
pub struct SnapshotManager {
    /// Per-file snapshots (legacy support): file_path -> snapshot
    snapshots: Arc<Mutex<HashMap<String, SnapshotData>>>,
    /// Per-change snapshots (for selective undo): change_id -> (file_path, snapshot)
    change_snapshots: Arc<Mutex<HashMap<String, (String, SnapshotData)>>>,
}
...
impl SnapshotManager {
    pub fn new() -> Self {
        Self {
            ...
            change_snapshots: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    /// Create a snapshot of the current file content (per-file, legacy)
    ///
    ...
    /// Create a snapshot for a specific change (for selective undo)
    ///
    /// This stores the file state BEFORE applying this change
    pub fn create_change_snapshot(&self, change_id: String, file_path: String, content: String) {
        let snapshot = SnapshotData::new(file_path.clone(), content);

        let mut change_snapshots = self.change_snapshots.lock().unwrap();
        change_snapshots.insert(change_id, (file_path, snapshot));
    }

    /// Get snapshot for a file (if exists) - legacy per-file
    pub fn get_snapshot(&self, file_path: &str) -> Option<SnapshotData> {
        let snapshots = self.snapshots.lock().unwrap();
        snapshots.get(file_path).cloned()
    }

    /// Get snapshot for a specific change
    pub fn get_change_snapshot(&self, change_id: &str) -> Option<(String, SnapshotData)> {
        let change_snapshots = self.change_snapshots.lock().unwrap();
        change_snapshots.get(change_id).cloned()
    }
    ...
    /// Remove snapshot for a specific change
    #[allow(dead_code)]
    pub fn remove_change_snapshot(&self, change_id: &str) {
        let mut change_snapshots = self.change_snapshots.lock().unwrap();
        change_snapshots.remove(change_id);
    }

    /// Clear all snapshots (both per-file and per-change)
    #[allow(dead_code)]
    pub fn clear_all(&self) {
        let mut snapshots = self.snapshots.lock().unwrap();
        snapshots.clear();
        let mut change_snapshots = self.change_snapshots.lock().unwrap();
        change_snapshots.clear();
    }
    ...
    pub fn count(&self) -> usize {
        let snapshots = self.snapshots.lock().unwrap();
        let change_snapshots = self.change_snapshots.lock().unwrap();
        snapshots.len() + change_snapshots.len()
    }
}
